...
if region == "full":
    ...
    points = generate_parameter_points(max_M1, max_M2, N, N)
...
if region == "compressed":
    ...
    for x in np.linspace(min_M1, max_M1, N):
        ...
        for y in np.logspace(np.log10(mindiff), np.log10(max_diff), N):
...
working_directory = "checkmate_%i_%i_%i" % (mu, tanB, m_sleptons)
#f(optimize_higgs())
...
print("\t Run micromegas: %s" % str(use_micromegas))
print("\t Run checkmate: %s" % str(use_checkmate))
print("\t Remake: %s\n" % str(remake))
print("Creating %i Processes..." % len(points))

queue = run(points, remake, use_prospino, use_micromegas, use_checkmate, working_directory)
data = []
while queue.qsize() > 0:
    ret = queue.get()
    data.append([ret[i] for i in ["m_x1", "m_x2", "cx", "gm2", "omega", "r"]])
    
m_x1, m_x2, cx, gm2, om, r = np.array(data).T  
m_x1 = np.abs(m_x1)
m_x2 = np.abs(m_x2)
...
sel = ~np.isnan(m_x2) * ~np.isnan(m_x1) * ~np.isnan(r)

print(r)

if use_checkmate:
    data = np.array([m_x1, m_x2, r]).T[sel]
    if region == "compressed":
        data[:, 0] = data[:, 1] - data[:,0]
    
    
    compare = np.array([m_x1, m_x2, 1*np.ones(len(r))]).T
    if region == "compressed":
        compare[:,0] = compare[:,1] - compare[:,0]
    
    draw_contour( compare, data, cmap=cmaps[2], alpha=0.1, show_pointlabels=True)
    
    s = data.T[2] > 1
    plt.scatter(data.T[1][s], data.T[0][s], color="black")
